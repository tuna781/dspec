#!/usr/bin/env node
// Summarise a results directory: per task and condition, the rubric score, time and cost.
...
for (const f of fs.readdirSync(dir).filter((f) => /^[^.]+\.(with|without)\.\d+\.json$/.test(f))) {
  ...
  const cell = (cells[`${task}|${cond}`] ??= { task, cond, scores: [], secs: [], cost: [] });
  cell.scores.push(score);
  if (run) { cell.secs.push(run.duration_ms / 1000); cell.cost.push(run.total_cost_usd); }
}

const mean = (xs) => { const v = xs.filter((x) => typeof x === 'number'); return v.length ? v.reduce((a, b) => a + b, 0) / v.length : NaN; };
console.log('| task | map | scores (0–2) | mean score | mean time | mean cost |');
console.log('|---|---|---|---|---|---|');
for (const c of Object.values(cells).sort((a, b) => a.task.localeCompare(b.task) || b.cond.localeCompare(a.cond))) {
  console.log(`| ${c.task} | ${c.cond} | ${c.scores.map((s) => s ?? '?').join(' ')} | ${mean(c.scores).toFixed(2)} | ${mean(c.secs).toFixed(0)}s | $${mean(c.cost).toFixed(2)} |`);
}
