// ============================================================
// Asking the user which agents to install into
//
// ⚠️ **Never ask when asking is not allowed.** A `dspec init` that hangs in CI waiting for a
// keypress is a far worse failure than a skipped question: it reports nothing and times out ten
// minutes later. `--yes` and a non-TTY stdin are both checked by the caller.
// ============================================================
 ...
 * `1,3` works over ssh, inside tmux, and in every editor's embedded shell — and it costs one line
 * of explanation.
 *
 * Empty input takes the preselected set, so Enter is always a safe answer.
 */
