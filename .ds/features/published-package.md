...
- **`.agents/` and `AGENTS.md` are untracked, though `.gitignore` says they are committed.**
  Whether they should be added or the comment corrected is not answerable from the code: only the
  Claude Code half is tracked today, and both halves are regenerated output.
